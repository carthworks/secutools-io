...
`;
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-teal-50 to-green-50 dark:from-slate-950 dark:via-emerald-950/20 dark:to-slate-950 p-4 sm:p-6 lg:p-8 text-slate-900 dark:text-slate-100">
            <div className="max-w-7xl mx-auto space-y-6">
                {/* Header */}
                <div className="text-center space-y-3">
                    <div className="inline-flex items-center gap-2 px-4 py-2 bg-white/60 dark:bg-slate-900/80 backdrop-blur-sm rounded-full border border-emerald-200 dark:border-emerald-900/50 shadow-sm">
                        <Terminal className="w-5 h-5 text-emerald-600 dark:text-emerald-400" />
                        <span className="text-sm font-medium text-emerald-900 dark:text-emerald-300">Penetration Testing</span>
                    </div>
                    <h1 className="text-4xl sm:text-5xl font-bold bg-gradient-to-r from-emerald-600 via-teal-600 to-green-600 dark:from-emerald-400 dark:via-teal-400 dark:to-green-400 bg-clip-text text-transparent">
                ...
                {/* Warning Banner */}
                <div className="bg-amber-50 dark:bg-amber-950/40 border-2 border-amber-200 dark:border-amber-900/50 rounded-xl p-4">
                ...
                {/* Category Tabs */}
                <div className="bg-white/70 dark:bg-slate-900/80 backdrop-blur-xl rounded-2xl shadow-xl border border-white/20 dark:border-slate-800 p-6">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <button
                            ...
                            className="px-4 py-2 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white rounded-lg text-sm font-semibold hover:shadow-lg transition-all flex items-center gap-2"
                        >
                        ...
                        {categories.map((cat) => (
                            <button
                                ...
                                className={`p-4 rounded-xl border-2 transition-all ${selectedCategory === cat.id
                                        ...
                                        : "border-slate-200 dark:border-slate-700/80 hover:border-emerald-300 dark:hover:border-emerald-600 bg-white dark:bg-slate-800/80"
                                    }`}
                            >
                                <cat.icon className={`w-6 h-6 mx-auto mb-2 ${selectedCategory === cat.id ? "text-emerald-600 dark:text-emerald-400" : "text-slate-400"
                                    }`} />
                                <div className={`text-sm font-semibold ${selectedCategory === cat.id ? "text-emerald-900 dark:text-emerald-300" : "text-slate-600 dark:text-slate-300"
                                    }`}>
                                    {cat.label}
                                </div>
                    ...
                    <div className="mt-4">
                        <input
                            ...
                            className="w-full px-4 py-3 rounded-xl border-2 border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:border-emerald-500 focus:ring-4 focus:ring-emerald-100 dark:focus:ring-emerald-950/50 outline-none transition-all"
                        />
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    {filteredPayloads.map((payload, idx) => (
                        <div
                            key={idx}
                            className="bg-white/70 dark:bg-slate-900/80 backdrop-blur-xl rounded-xl shadow-lg border border-white/20 dark:border-slate-800 overflow-hidden hover:shadow-xl transition-all"
                            style={{ animation: `slideIn 0.3s ease-out ${idx * 0.05}s both` }}
                        >
                            <div className="p-5 space-y-3">
                                <div className="flex items-start justify-between gap-3">
                                    <div className="flex-1">
                                        <h3 className="font-semibold text-slate-800 dark:text-slate-200 mb-1">{payload.name}</h3>
                                        <p className="text-sm text-slate-600 dark:text-slate-400">{payload.description}</p>
                                    </div>
                                ...
                                <div className="bg-slate-900 dark:bg-slate-950 border border-slate-800 rounded-lg p-4 relative group">
                ...
                {filteredPayloads.length === 0 && (
                    <div className="text-center py-12">
                        ...
                        <p className="text-slate-600 dark:text-slate-400">No payloads found matching your search.</p>
                    </div>
                )}

                {/* Info Card */}
                <div className="bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm rounded-xl border border-slate-200 dark:border-slate-800 p-6">
                    <div className="flex items-start gap-3">
                        <Shield className="w-5 h-5 text-emerald-600 dark:text-emerald-400 flex-shrink-0 mt-0.5" />
                        <div className="text-sm text-slate-600 dark:text-slate-400">
                            <strong className="text-slate-900 dark:text-slate-100">Prevention Tips:</strong> Always validate and sanitize user input.
