        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          <div>
            ...
            <textarea
              ...
              className="w-full min-h-[220px] border border-slate-300 dark:border-slate-700 rounded-lg p-3 font-mono text-sm bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500"
            ...
            <div className="flex items-center gap-2 mt-3 flex-wrap">
              <button
                ...
                className="px-3.5 py-1.5 border border-emerald-300 dark:border-emerald-700 rounded-lg bg-emerald-50 dark:bg-emerald-950/60 hover:bg-emerald-100 dark:hover:bg-emerald-900/60 text-emerald-700 dark:text-emerald-300 font-medium text-sm flex items-center gap-1.5 transition shadow-sm"
              ...
              <div className="border-l border-slate-300 dark:border-slate-700 pl-3 ml-auto flex items-center gap-2">
                <label htmlFor="mode-select" className="text-xs text-slate-500 dark:text-slate-400">Mode</label>
                <select
                  id="mode-select"
                  value={mode}
                  onChange={(e) => setMode(e.target.value as Mode)}
                  className="text-xs p-1.5 border border-slate-300 dark:border-slate-700 rounded-md bg-white dark:bg-slate-950 text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500"
            ...
            <div className="mt-2 text-sm">
              ...
              {analysis.error && <div className="mt-2 text-xs text-rose-600 dark:text-rose-400 font-medium">Error: {analysis.error}</div>}
              {message && (
                <div
                  className={`mt-2 text-sm font-medium ${message.type === "err" ? "text-rose-600 dark:text-rose-400" : "text-emerald-600 dark:text-emerald-400"}`}
                  role="alert"
                >
                  {message.text}
                </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <div className="text-sm font-semibold text-slate-900 dark:text-slate-100">Analysis Preview</div>
              <div className="flex items-center gap-1.5 flex-wrap">
                <button
                  onClick={handleCopy}
                  className="px-2.5 py-1 border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-md text-xs flex items-center gap-1 transition"
            ...
            <div
              ref={previewRef}
              className={`w-full min-h-[220px] border border-slate-200 dark:border-slate-800 rounded-lg p-3 text-sm bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 overflow-auto ${
            ...
            <div className="mt-2 flex items-center justify-between">
              <div className="text-xs text-slate-500 dark:text-slate-400">Analyzed locally — no data sent to server</div>
